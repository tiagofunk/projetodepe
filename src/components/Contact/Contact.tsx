...
export const Contact = () => {
  ...
  const serviceTypeTitle = "Atividade desejada";
  ...
  const voluntary = "Voluntariado";
  const wantToBeAttended = "Atendimento";
  const donations = "Doação de material";
  ...
  const optionsTypeService = ["D'Favela", "D'Votar", "D'Reciclar", "D'Eternizar", "D'Raiz", "D'Lei", "D'Sorrir", "D'Parir"];

  const optionsDonations = [
    "Móveis",
    "Eletrodomesticos",
    "Roupas",
    "Instrumentos",
  ...
  function validateValues() {
    ...
    if (!validVoluntaryFieldsIfNecessary()) isValid = false;
    if (!validWantToBeAttendedFieldsIfNecessary()) isValid = false;
    if (!validDonationsFieldsIfNecessary()) isValid = false;
    return isValid;
  }
  ...
  function validVoluntaryFieldsIfNecessary() {
    ...
    if (formValues.subject === voluntary) {
      if (
        !validate(
          formValues.occupationArea,
          regexNotEmptyString,
          "occupationArea",
          occupationAreaFailMessage
        )
      )
        isValid = false;
      if (
        !validate(
          formValues.howYouWantToContribute,
          regexNotEmptyString,
          "howYouWantToContribute",
          howYouWantToContributeFailMessage
        )
      )
        isValid = false;
      if (
        !validate(
          formValues.daysAndTimesAvailable,
          regexNotEmptyString,
          "daysAndTimesAvailable",
          daysAndTimesAvailableFailMessage
        )
      )
        isValid = false;
    }
  ...
  function validDonationsFieldsIfNecessary() {
    ...
    if (formValues.subject === donations) {
      ...
      if (
        !validate(
          formValues.quantityDonation,
          regexNotEmptyString,
          "quantityDonation",
          quantityDonationFailMessage
        )
      )
        isValid = false;
    }
    ...
    return (
      <>
        ...
        {errors[field as keyof typeof errors] && (
          <ErrorText>{errors[field as keyof typeof errors]}</ErrorText>
        )}
      </>
    );
  }
  ...
  function getMedicalAppointmentFields() {
    return (
      <>
        <HorizontalBox>
          <VerticalBox>
            {getFieldCheck(serviceTypeTitle, "serviceType", optionsTypeService)}
          </VerticalBox>
        </HorizontalBox>
      </>
    );
  ...
  function getDonationsFields() {
    return (
      <>
        <HorizontalBox>
          <VerticalBox>
            {getFieldCheck(donationTypeTitle, "donationType", optionsDonations)}
          </VerticalBox>
          <VerticalBox>
            {getField(quantityDonationTitle, "quantityDonation")}
          </VerticalBox>
        </HorizontalBox>
      </>
    );
    ...
    <Container>
      ...
      >
        <VerticalBox>
          ...
          <HorizontalBox>
            <VerticalBox>
              {getFieldCheck(subjectTitle, "subject", optionsSubject)}
            </VerticalBox>
